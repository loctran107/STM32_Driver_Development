...
uint8_t Txdata[32] = "Testing I2C Slave Tx successful\n";
...
void I2C_ApplicationEventCallBack(I2C_Handle_t* pI2CHandler, uint8_t appEvt) {
	...
	case I2C_EVT_DATA_REQ:	if (commandCode ==  COMMAND_REQUEST) {
								I2C_SlaveSendData(I2C1, strlen((char*) Txdata));
							} else if (commandCode == COMMAND_READ_DATA) {
								I2C_SlaveSendData(I2C1, Txdata[counter++]);
							} else {
	...
	case I2C_ERR_AF:		commandCode = 0xff; counter = 0; break;

	//End I2C Slave Reception mode
	case I2C_EVT_STOPF_CMPLT: break;
